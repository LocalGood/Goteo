    ...
    class Skillmatching_accounts {
        ...
        public static function process ($action = 'list', $id = null, $filters = array()) {
           ...
           if ($action == 'move') {
                ...
                $projectData = Model\Skillmatching::getMini($original->project);
                ...
                if ($original->method == 'paypal' || $original->status != 1) {
                    ...
                    throw new Redirection('/admin/skillmatching_account');
                }
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['move']) ) {
                    ...
                    if ($invest->save($errors)) {
                        ...
                        if (!$original->resign) {
                            // sacar recompensas
                            $rewards = Model\Skillmatching\Reward::getAll($projectNew, 'individual');
                        ...
                        /*if ($original->setStatus('5')) {
                            ...
                            $log->populate('Aporte reubicado', '/admin/skillmatching_account',
           ...
           if ($action == 'update') {
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update']) && isset($new) && isset($status[$new])) {
                    ...
                    throw new Redirection('/admin/skillmatching_account/details/'.$id);
                }
           ...
           if ($action == 'solve') {
                ...
                $projectData = Model\Skillmatching::getMini($invest->project);
                ...
                $log = new Feed();
                $log->setTarget($projectData->id);
                $log->populate('Cargo cancelado manualmente (admin)', '/admin/skillmatching_account',
                ...
                if ($invest->solve($errors)) {
                    ...
                    $log = new Feed();
                    $log->setTarget($projectData->id);
                    $log->populate('Incidencia resuelta (admin)', '/admin/skillmatching_account',
                    ...
                    $log = new Feed();
                    $log->setTarget($projectData->id);
                    $log->populate('Fallo al resolver incidencia (admin)', '/admin/skillmatching_account',
           ...
           if ($action == 'add') {
                ...
                $projects = Model\Skillmatching::active(false, true);
                // usuarios
...
//@CALLSYS
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add']) ) {
                    ...
                    $projectData = Model\Skillmatching::getMini($_POST['project']);
...
//@CALLSYS
                    ...
                    if ($invest->save($errors)) {
                        ...
                        $log = new Feed();
                        $log->setTarget($projectData->id);
                        $log->populate('Aporte manual (admin)', '/admin/skillmatching_account',
                            \vsprintf("%s ha aportado %s al proyecto %s en nombre de %s", array(
                                Feed::item('user', $_SESSION['user']->name, $_SESSION['user']->id),
                                Feed::item('money', $_POST['amount'].' &yen;'),
                                Feed::item('project', $projectData->name, $projectData->id),
            ...
            if ($action == 'report') {
                // estados de aporte
                $project = Model\Skillmatching::get($id);
...
*/
            ...
            if (($action == 'execute' && $invest->status == 0) || ($action == 'execute' && $invest->status == 5)) {
                ...
                if (!empty($log_text)) {
                    ...
                    $log = new Feed();
                    $log->setTarget($project->id);
                    $log->populate('Cargo ejecutado manualmente (admin)', '/admin/skillmatching_account',
                        \vsprintf($log_text, array(
                            Feed::item('user', $_SESSION['user']->name, $_SESSION['user']->id),
                            Feed::item('user', $userData->name, $userData->id),
                            Feed::item('money', $invest->amount.' &yen;'),
            ...
            if ($action == 'resign' && !empty($id) && $_GET['token'] == md5('resign')) {
                if ($invest->setResign(true)) {
                    ...
                    throw new Redirection('/admin/skillmatching_account/detail/'.$invest->id);
                } else {
            ...
            $procStatus = Model\Skillmatching::procStatus();
            // estados de aporte
            $investStatus = Model\Invest::status();
...
//                }
            ...
            if ($filters['filtered'] == 'yes') {
                $list = Model\Invest::getList($filters, null, 999, 'skillmatching');
            } else {
                $list = array();
            }
