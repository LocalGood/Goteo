...
jQuery(document).ready(function ($) {
    ...
    $('.evaluate-criteria').change(function () {
        success_text = $.ajax({async: false, type: "POST", data: ({campo: this.name, valor:this.checked}), url: '<?php echo LG_BASE_URL_GT; ?>/ws/set_review_criteria/<?php echo $_SESSION['user']->id; ?>/<?php echo $review->id; ?>'}).responseText;
        $("#total-score").html(success_text);
    });

    $('.evaluate-comment').change(function () {
        success_text = $.ajax({async: false, type: "POST", data: ({campo: this.name, valor:this.value}), url: '<?php echo LG_BASE_URL_GT; ?>/ws/set_review_comment/<?php echo $_SESSION['user']->id; ?>/<?php echo $review->id; ?>'}).responseText;
