...
$pagedResults = new \Paginated($this['skillmatchings'], 10, isset($_GET['page']) ? $_GET['page'] : 1);
?>
<a href="/admin/translates" class="button"><?php echo Text::_("Asignar traductores"); ?></a>

<div class="widget board">
    <form id="filter-form" action="/admin/skillmatchings" method="get">
    ...
    <a href="/admin/skillmatchings/?reset=filters"><?php echo Text::_("Quitar filtros"); ?></a>
<?php if ($filters['filtered'] != 'yes') : ?>
    <p><?php echo Text::_("Es necesario poner algun filtro, hay demasiados registros!"); ?></p>
<?php elseif (empty($this['skillmatchings'])) : ?>
...
<?php if (!empty($this['skillmatchings'])) :
        ...
        <tbody>
            ...
            <tr>
                <td colspan="7">
                    ...
                    <?php if ($project->status < 3 && $project->status > 0) : ?><a href="<?php echo "/admin/skillmatchings/publish/{$project->id}"; ?>" onclick="return confirm('<?php echo Text::_("El proyecto va a comenzar los 40 dias de la primera ronda de campaña, ¿comenzamos?"); ?>');">[<?php echo Text::_("Publicar"); ?>]</a><?php endif; ?>
                    <?php if ($project->status != 1) : ?><a href="<?php echo "/admin/skillmatchings/enable/{$project->id}"; ?>" onclick="return confirm('<?php echo Text::_("Mucho Ojo! si el proyecto esta en campaña, ¿Reabrimos la edicion?"); ?>');">[<?php echo Text::_("Reabrir edición"); ?>]</a><?php endif; ?>
                    <?php if ($project->status == 4) : ?><a href="<?php echo "/admin/skillmatchings/fulfill/{$project->id}"; ?>" onclick="return confirm('<?php echo Text::_("El proyecto pasara a ser un caso de éxito, ok?"); ?>');">[<?php echo Text::_("Retorno Cumplido"); ?>]</a><?php endif; ?>
                    <?php if ($project->status == 5) : ?><a href="<?php echo "/admin/skillmatchings/unfulfill/{$project->id}"; ?>" onclick="return confirm('<?php echo Text::_("Lo echamos un paso atras, ok?"); ?>');">[<?php echo Text::_("Retorno Pendiente"); ?>]</a><?php endif; ?>
                    <?php if ($project->status < 3 && $project->status > 0) : ?><a href="<?php echo "/admin/skillmatchings/cancel/{$project->id}"; ?>" onclick="return confirm('<?php echo Text::_("El proyecto va a desaparecer del admin, solo se podra recuperar desde la base de datos, Ok?"); ?>');">[<?php echo Text::_("Descartar"); ?>]</a><?php endif; ?>
                </td>
            </tr>
            <tr>
                <td colspan="5">
                    ...
                    <a href="/admin/skillmatchings/images/<?php echo $project->id; ?>">[<?php echo Text::_("Organizar imágenes"); ?>]</a>
